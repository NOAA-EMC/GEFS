...
c                       divide by zero.
c   12-10-19  Ota       Added an option to use sigma file directly.
c
c Input files:
...
c
      ...
      USE trig_vals; USE atcf; USE sig_parms; USE sig2p; USE grid_bounds
c
...
c
      ...
      integer reslch(maxtime)
      character*500 sigfile
c
...
c
      ...
      call read_nlists (inp,stormswitch,ifhours,reslch)
     ...
     &                   ,slonfg,slatfg,iret)
      ...
      if (sigpath /= '') then  ! from sigma files
        write(sigfile,'(a,i2.2)') trim(adjustl(sigpath)),ifhours(1)
     ...
     & '!!! WARNING: in init_sig2p, try p-grib file instead. rc= ',iret
           ...
           imax = nllon
           jmax = nllat
           call getsigmainfo(imax,jmax,grdspc,igetgds)
        end if
      end if

      if (sigpath == '' .or. iret /= 0) then  ! from p-grib files
...
c     reslch     array containing the resolution change for sigma file
...
c
      ...
      character*500 sigfile,sfluxfile
...
c
        ...
        readflag = .FALSE.
        if(.not. fsig_init) then
           call getdata (readflag,valid_pt,imax,jmax
     &                  ,ifhours(ifh),inp)
        else
           valid_pt = .false.
           if(ifcsthour < 100) then
              write(sigfile,'(a,i2.2)') trim(adjustl(sigpath)),ifcsthour
           else
              write(sigfile,'(a,i3.2)') trim(adjustl(sigpath)),ifcsthour
           end if
           plev(1) = log(85.0)
           plev(2) = log(70.0)
           plev(3) = log(50.0)
           if(reslch(ifh) == 1) call init_sig2p(lugb,sigfile, 
     &        ddeg,500.0e+2,iret)
           print *,'Read ',trim(adjustl(sigfile))
           if(sfluxpath /= '') then
              if(ifcsthour < 100) then
                 write(sfluxfile,'(a,i2.2)') trim(adjustl(sfluxpath)),
     &              ifcsthour
              else
                 write(sfluxfile,'(a,i3.2)') trim(adjustl(sfluxpath)),
     &              ifcsthour
              end if
             call oper_sig2p(lugb,sigfile,plev,iret,sfluxfile)
           else
             call oper_sig2p(lugb,sigfile,plev,iret)
           end if
           if(iret == 0) then  ! All or nothing when using sigma file
              readflag = .true.
              valid_pt = .true.
              readflag(1) = .false.
              readflag(2) = .false.
           else
              print *,'Sigma file read error : rc=',iret
           end if
        end if
...
c             flag will have a value of 'U', for "undetermined".
     ...
     &              calcparm(1,ist) .and. stormswitch(ist) == 1) then
                  ...
                  if (atcfname(1:4) == 'AVNO' .and.
     &                abs(slatfg(ist,ifh)) >= 25.0) then
                    max_mslp_850 = 405.0
                  else if (atcfname(1:4) == 'AVNO' .and.
     &                abs(slatfg(ist,ifh)) < 25.0) then
                    max_mslp_850 = 405.0
                  else
                    max_mslp_850 = 323.0
...
c           gotten to this point in threshloop).
        ...
        enddo threshloop
        print *,'dtemp:',allocated(dtemp),size(dtemp)
        print *,'isortix:',allocated(isortix),size(isortix)
        print *,'iwork:',allocated(iwork),size(iwork)
...
c
     ...
     &      atcfname(1:3) == 'EMX' .or. atcfname(1:3) == 'PAR' ) then
          errmax  = err_avn_init
...
c
c---------------------------------------------------------------------
      subroutine read_nlists (inp,stormswitch,ifhours,reslch)
c
...
c
      USE inparms; USE set_max_parms; USE atcf; USE sig_parms
c
...
c
      ...
      namelist/siginfo/sigpath,sfluxpath,ddeg,reslch
c
...
c
c---------------------------------------------------------------------
      subroutine getsigmainfo(imax,jmax,grdspc,igetgds)
c
c     ABSTRACT: This routine produces grid information of sigma files
c     instead of the p-grib files.
c
      USE sig_parms; USE grid_bounds
      implicit none
      integer imax,jmax
      real    grdspc
      integer igetgds(200)
...
c
      ...
      allocate(glat(jmax))
      do i=1,imax
        glon(i) = float(i-1) * ddeg
      enddo
      do j=1,jmax
        glat(j) = 90.0 - float(j-1) * ddeg
      enddo
      igetgds(1) = 0
      igetgds(2) = imax
      igetgds(3) = jmax
      ...
      igetgds(6) = 0
      igetgds(7) = -90000
      igetgds(8) = 360000-nint(ddeg*1000.0)
      igetgds(9) = nint(ddeg*1000.0)
      igetgds(10) = nint(ddeg*1000.0)
      igetgds(11) = 0
      igetgds(12) = 255
c
      return
