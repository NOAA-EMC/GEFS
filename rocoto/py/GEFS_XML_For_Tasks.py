...
def get_param_of_task(dicBase, taskname):
    ...
    if sVarName in dicBase:
        ...
        walltime = dicBase[sVarName.upper()]
    ...
    if sVarName_nodes in dicBase:
        ...
        if sVarName_ppn in dicBase:
            ...
            if sVarName_tpp in dicBase:
                ...
                nodes = "{0}:ppn={1}:tpp={2}".format(dicBase[sVarName_nodes], dicBase[sVarName_ppn],

                                                     dicBase[sVarName_tpp])

            else:

                nodes = "{0}:ppn={1}".format(dicBase[sVarName_nodes], dicBase[sVarName_ppn])

        else:

            if sVarName_tpp in dicBase:

                nodes = "{0}:tpp={1}".format(dicBase[sVarName_nodes], dicBase[sVarName_tpp])

            else:

                nodes = "{0}".format(dicBase[sVarName_nodes])
    ...
    if taskname == 'jgefs_forecast_high':
        ...
        nodes = "{0}:ppn={1}:tpp={2}".format(iNodes, iPPN, iTPP)



    return walltime, nodes, memory



def create_metatask_task(dicBase, taskname="jgefs_init_fv3chgrs", sPre="\t", GenTaskEnt=False, WHERE_AM_I="cray"):
    ...
    walltime, nodes, memory = get_param_of_task(dicBase, taskname)
    ...
    if taskname in metatask_names:
        ...
        strings += create_metatask(taskname=taskname, jobname=jobname, walltime=walltime, nodes=nodes,

                                   memory=memory, sPre=sPre, GenTaskEnt=GenTaskEnt,WHERE_AM_I=WHERE_AM_I)

    else:

        strings += create_task(taskname=taskname, jobname=jobname, walltime=walltime, nodes=nodes,
...
def get_jobname(taskname):
    ...
    # dic_jobnames["jgefs_enkf_track"] = "&EXPID;@Y@m@d@H010"

    # # - 02

    # dic_jobnames["jgefs_init_separate"] = "&EXPID;@Y@m@d@H012"

    # # - 03

    # dic_jobnames["jgefs_init_process"] = "&EXPID;@Y@m@d@H013"

    # # - 04

    # dic_jobnames["jgefs_init_combine"] = "&EXPID;@Y@m@d@H014"

    # # - 05

    # dic_jobnames["jgefs_init_fv3chgrs"] = "&EXPID;@Y@m@d@H15"  #_#member#"

    # # - 06

    # dic_jobnames["jgefs_forecast_high"] = "&EXPID;@Y@m@d@H100" #_#member#"

    # # - 07

    # dic_jobnames["jgefs_post_high"] = "&EXPID;@Y@m@d@H102" #_#member#"

    # # - 08

    # dic_jobnames["jgefs_prdgen_high"] = "&EXPID;@Y@m@d@H103" #_#member#"

    # # - 09

    # dic_jobnames["jgefs_ensstat_high"] = "&EXPID;@Y@m@d@H104"

    # # - 10

    # dic_jobnames["jgefs_post_track"] = "&EXPID;@Y@m@d@H396"

    # # - 11

    # dic_jobnames["jgefs_post_genesis"] = "&EXPID;@Y@m@d@H397"

    # # - 12

    # dic_jobnames["jgefs_enspost"] = "&EXPID;@Y@m@d@H395"

    #

    # jobname = dic_jobnames[taskname]
...
def create_metatask( \
        ...
        jobname="&EXPID;@Y@m@d@H15_#member#", walltime="00:30:00", \

        nodes="1:ppn=12:tpp=2", memory="600M", sPre="\t", GenTaskEnt=False, \
    ...
    if taskname == "jgefs_init_fv3chgrs":
        ...
        strings += sPre + '\t\t' + '<join><cyclestr>&LOG_DIR;/@Y@m@d/gefs_#member#{0}@H.%J</cyclestr></join>\n'.format(
...
def create_task( \
        ...
        jobname="&EXPID;@Y@m@d@H010", walltime="01:50:00", \

        nodes="2:ppn=20", memory="3000M", sPre="\t", GenTaskEnt=False, \
...
def get_DATE_VARS(sPre="\t\t"):
    ...
    for sKey in dicDATE_VARS:
        ...
        # sDATE_VARS += sPre + '<envar>\n'

        # sDATE_VARS += sPre + '\t<name>{0}</name>\n'.format(sKey)

        # sDATE_VARS += sPre + '\t<value><cyclestr>{0}</cyclestr></value>\n'.format(dicDATE_VARS[sKey])
...
def get_ENV_VARS(sPre="\t\t"):
    ...
    for sKey in dicENV_VARS:
        ...
        # sENV_VARS += sPre + '<envar>\n'

        # sENV_VARS += sPre + '\t<name>{0}</name>\n'.format(sKey)

        # sENV_VARS += sPre + '\t<value>{0}</value>\n'.format(dicENV_VARS[sKey])
