...
#mem_per_group=5
DoLR=yes
DoAllMEMs=no # "yes" means doing all members in one time
...
do
    ...
    if [[ $DoAllMEMs == "yes" ]]; then
        ...
        if (( imem > num_mems_for_first_group )); then
            ...
            #    fcst[$imem]=$(qsub -W depend=afterok:$getcfssst:$atmos_init:${wave_prep[$imem]}:${wave_prep[1]} jgefs_forecast.ecf_$mem)
            #fi
            fcst[$imem]=$(qsub -W depend=afterok:$getcfssst:$atmos_init:${wave_prep[$imem]}:${wave_prep[1]}:${fcst[$imem_adj]} jgefs_forecast.ecf_$mem)
        else
            if [[ $imem == 1 ]]; then
                fcst[$imem]=$(qsub -W depend=afterok:$getcfssst:$atmos_init:${wave_prep[$imem]} jgefs_forecast.ecf_$mem)
            else
