...
for tz in t00z t06z t12z t18z; do
	...
	while (( np <= npair )); do
		if [[ ! -s finn.pair${iip} ]]; then
			...
			ln -s -f  $FIXgefs/gefs.pertback.${tz}.n${np}.pair${iip} finn.pair${iip}
		fi # [[ ! -s finn.pair${iip} ]]
		if [[ ! -s finp.pair${iip} ]]; then
			# ln -s -f  gefs.pertback.${tz}.p${np}.pair${iip} finp.pair${iip}
			ln -s -f  $FIXgefs/gefs.pertback.${tz}.p${np}.pair${iip} finp.pair${iip}
		fi # [[ ! -s finp.pair${iip} ]]
		(( ip = ip + 1 ))
		(( iip = ip ))
		if (( iip < 10 )); then
...
done # for tz in t00z t06z t12z t18z
#
...
if [[ $netflag == 0 && $mskflag == 0 ]]; then
	...
	(( recentpair = recentmem / 2 ))
	(( ipairh = ipair0 + recentpair ))

	for ipair in $nlist; do
		(( jpair = ipair - ipair0 ))
#echo DHXXXX $ipair $ipair0 $jpair $recentpair $recentmen
		if (( jpair < 0 )); then
                        (( jpair = jpair + num_pair_enkf ))
                fi
		if (( jpair > 0 && jpair <= recentpair )); then
			(( jpairn = jpair ))
			(( jpairp = jpair + recentpair ))
			if (( jpairn < 10 )); then
				jpairn=0$jpairn
			fi
			if  (( jpairp < 10 )); then
			...
			ln -s finn.pair${ipair} fin_mem0$jpairn
			ln -s finp.pair${ipair} fin_mem0$jpairp
		else
			cp -pr finn.pair${ipair} sanlgm${ipair}n
			cp -pr finp.pair${ipair} sanlgm${ipair}p
		fi # (( ipair > ipair0 && ipair <= ipairh))
	done # for ipair in $nlist
	$APRUN $GETATMENSMEANEXEC ./ fin_ensmean fin $recentmem 6 | tee -a $pgmout
	export err=$?
	err_chk
	echo `date` $GETATMENSMEANEXEC after

	$APRUN $RECENATMPEXEC fin fin_ensmean sanl.in.pair01 sanlgm $recentmem | tee -a $pgmout
	export err=$?
	err_chk
	echo `date` $RECENATMPEXEC after
        ...
        for ipair in $nlist; do
                ...
                if (( jpair > 0 && jpair <= recentpair )); then
                        ...
                        cp -pr  sanlgm_mem0$jpairn sanlgm${ipair}n
                        cp -pr  sanlgm_mem0$jpairp sanlgm${ipair}p
                fi # (( ipair > ipair0 && ipair <= ipairh))
        done # for ipair in $nlist

else # [[ $netflag == 0 && $mskflag == 0 ]]
	if [[ $parallelinit = no ]]; then
...
for ip in $nlist; do
	...
	fi # [[ -s sanlgm${ip}n && -s sanlgm${ip}p ]]
done # for ip in $nlist
#
echo
