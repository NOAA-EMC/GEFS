...
restart_base_pattern = "{com_root}/{net}/{ver}/{run}.%Y%m%d/%H/{component}/restart"  # Time of previous run
...
fcst_file_pattern = "{com_root}/{net}/{ver}/{run}.%Y%m%d/%H/{component}/sfcsig/ge{member}.t%Hz.atmf{forecast_hour:03}.nemsio"
...
def main() -> None:
    ...
    if (init_type == "warm"):
        prev_fcst_file = get_previous_forecast(time=time, incr=incr, max_lookback=max_lookback, com_root=com_root, net=net, ver=ver, run=run)
        ...
        analysis_filename = regrid_analysis(time=time, regrid_aprun=regrid_aprun, regrid_exec=regrid_exec,
                                            com_root=com_root, com_gfs=com_gfs, net=net, ver=ver, run=run,
        ...
        restart_files = get_all_restart_files(time=time, incr=incr, max_lookback=max_lookback, com_root=com_root, net=net, ver=ver, run=run)
    ...
    if(init_type == "cold"):
        ...
        restart_files = get_tracer_restart_files(time=time, incr=incr, max_lookback=max_lookback, com_root=com_root, net=net, ver=ver, run=run)
...
def get_previous_forecast(time: datetime, incr: int, max_lookback: int, com_root: str, net: str, ver: str, run: str) -> str:
    for lookback in map(lambda i: incr * (i + 1), range(max_lookback)):
        last_time = time - timedelta(hours=lookback)
        prev_fcst_file = last_time.strftime(fcst_file_pattern.format(com_root=com_root, net=net, ver=ver, run=run, member="aer", component="chem", forecast_hour=lookback))
...
def get_tracer_restart_files(time: datetime, incr: int, max_lookback: int, com_root: str, net: str, ver: str, run: str) -> typing.List[str]:
    for lookback in map(lambda i: incr * (i + 1), range(max_lookback)):
        last_time = time - timedelta(hours=lookback)
        restart_base = last_time.strftime(restart_base_pattern.format(com_root=com_root, net=net, ver=ver, run=run, member="aer", component="chem"))
...
def get_all_restart_files(time: datetime, incr: int, max_lookback: int, com_root: str, net: str, ver: str, run: str) -> typing.List[str]:
    for lookback in map(lambda i: incr * (i + 1), range(max_lookback)):
        last_time = time - timedelta(hours=lookback)
        restart_base = last_time.strftime(restart_base_pattern.format(com_root=com_root, net=net, ver=ver, run=run, member="aer", component="chem"))
...
def regrid_analysis(time: datetime, regrid_aprun: str, regrid_exec: str, com_root: str, com_gfs: str, net: str, ver: str, run: str, n_lat: int, n_lon: int, ref_file: str) -> str:
