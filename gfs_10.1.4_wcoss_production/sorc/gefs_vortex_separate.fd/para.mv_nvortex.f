...
C   PROGMMR: Dennis Keyser  ORG: NP22       DATE: 2000-07-20
C
...
!      COMMON /COEF5/NCNT,NCNT2
      ...
      CALL W3TAGB('RELOCATE_MV_NVORTEX',2000,0202,0068,'NP22')
C
...
!      IDSL=DUMMY(219)
     ...
     &                 ps=PSREF,pm=SLREF)
      ...
      do k=1,kmax
      print*,'k,SLREF=',k,SLREF(k)
      end do

      CALL HURR_MESS(IMAX,JMAX,GLON,GLAT)
...
c      if(NCNT.ne.(212+IKMAX))print*,'Wrong Data Read In'
     ...
     2                 HDAT,HDATA,PDAT,data1,PSLB,SLREF,
     3                 nvcd,idvc,idsl,vcrd) 
...
C
      ...
      SUBROUTINE HURR_MESS(IMAX,JMAX,GLON,GLAT)
...
c      PRINT*,'READ2 COUNT = ',NRED2
     ...
     1                       JHF,MTV,MTV1,MTV2,MTV3,
     2                       HDAT,HDATA,PDAT,data1,PSLB,SL,
     3                       nvcd,idvc,idsl,vcrd)
...
C
     ...
     1          HDATA,DKY,IG,IFLAG,PSLB,ZDATG,GLON,GLAT,PS2)
...
C
      RDIST2=AMDX*AMDX+AMDY*AMDY
      IF(RDIST2.LE.0.02)THEN
        PRINT*,'TIME= ',ITIM,'h, Model Center is not relocated'
      ELSE
        PRINT*,'TIME= ',ITIM,'h, vortex relocation is completed' 
      END IF
...
C
     ...
     1            HDATA,DM1,IS1,IFLAG,PSLB,ZDATG,GLON,GLAT,PS2)
