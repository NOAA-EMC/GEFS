...
for hour in $hours; do
	if [[ $do_analysis = YES ]] && (( hour == 0 )); then
		...
		while [ $ic -le $SLEEP_LOOP_MAX ]; do
			...
			if [ $ic -eq $SLEEP_LOOP_MAX ]; then
				...
				cat <<-EOF
					FATAL ERROR in $0 ($stream): Post data still missing for analysis after waiting ${SLEEP_TIME}s.
						Looked for the following files:
							$(if [[ $RUNMEM != "gegfs" ]]; then Control file: $mcfile $(if [[ -f $mcfile ]]; then echo "exists"; else; echo "doesn't exist"; fi); fi)
							Grib file:    $mafile $(if [[ -f $mafile ]]; then echo "exists"; else; echo "doesn't exist"; fi)
							Index file:   $mifile $(if [[ -f $mifile ]]; then echo "exists"; else; echo "doesn't exist"; fi)
					EOF
				export err=1;
				err_chk
				exit $err
	...
	while [ $ic -le $SLEEP_LOOP_MAX ]; do
		...
		if [ $ic -eq $SLEEP_LOOP_MAX ]; then
			...
			cat <<-EOF
				FATAL ERROR in $0 ($stream): Post data still missing for f$fhr after waiting ${SLEEP_TIME}s.
					Looked for the following files:
