...
then
  ...
  then
    ...
    then
      ($DBNROOT/bin/dbn_alert MODEL GEFS_LEGACY $job $COMOUT/$cyc/pgrba/${RUN}.${cycle}.pgrbaf$fhr$cfsuffix)&
    fi
    if test "$DO_LOW_RES" = 'YES' -a ` expr $fhr % 6 ` -eq 0 -a ! -n "$cfsuffix"
    then
      ($DBNROOT/bin/dbn_alert MODEL GEFS_LEGACY $job $ENS_COM/$cyc/pgrbalr/${RUN}.${cycle}.pgrbaf$fhr.2$cfsuffix)&
    fi
...
then
  ...
  then
    ...
    then
      ($DBNROOT/bin/dbn_alert MODEL GEFS_LEGACY $job $COMOUT/$cyc/pgrbb/${RUN}.${cycle}.pgrbbf$fhr$cfsuffix)&
    fi
    if test "$DO_LOW_RES" = 'YES' -a ` expr $fhr % 6 ` -eq 0 -a $fhr -ge 90 -a ! -n "$cfsuffix"
    then
      ($DBNROOT/bin/dbn_alert MODEL GEFS_LEGACY $job $ENS_COM/$cyc/pgrbblr/${RUN}.${cycle}.pgrbbf$fhr.2$cfsuffix)&
    fi
  fi
