...
if [[ $RUN = ge??? ]]; then
	if [[ $fhr -gt 90 ]]; then
		...
		if [ $fhrm3 -lt 10 ]; then fhrm3=0$fhrm3; fi
			...
			if [[ $fhr -le 90 && $cycsuffix = true ]]; then
				rm $COMOUT/$cyc/sflux/${RUN}.${cycle}.sfluxgrbf$fhr$cfsuffix
				rm $COMOUT/$cyc/sflux/${RUN}.${cycle}.sfluxgrbif$fhr$cfsuffix
				# remove the 3-hourly files as well
				fhrm3=$((fhr-3))
			...
			fi # [[ $fhr -le 90 && $cycsuffix = true ]]
		fi
	else 
		if [[ $fhr -gt 84 && $fhr -ne 90 ]] && [[ $fhr -ne 96 && $fhr -ne 120 ]]; then
			rm $COMOUT/$cyc/sfcsig/${RUN}.${cycle}.bf$fhr$cfsuffix
		fi
	fi # [[ $fhr -gt 90 ]]
...
if test "$SENDDBN" = 'YES' -a "$NET" = 'gens' -a ` expr $cyc % 12 ` -eq 0; then
	if test `echo $RUN | cut -c1-2` = "ge"; then
		MEMBER=`echo $RUN | cut -c3-5 | tr '[a-z]' '[A-Z]'`
		if [[ $fhr -ge 0 && $fhr -le 84 && ` expr $fhr % 6 ` -eq 0 && ! -n "$cfsuffix" ]]; then
