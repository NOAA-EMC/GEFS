...
!        COMMON /COEF3/FHOUR,DUMMY
     ...
     &                 VP(:,:,:),RHP(:,:,:),SHP(:,:,:),TP(:,:,:)      
        ...
        REAL(4), ALLOCATABLE :: PSIG(:,:,:)

        REAL(4), ALLOCATABLE :: SIREF(:),SLREF(:),PD1(:)
        ...
        ALLOCATE ( SIREF(KMAX1),SLREF(KMAX),PD1(KMAX) )

        PSREF=1.E5

        CALL sigio_modpr(1,1,KMAX,nvcd,idvc,idsl,vcrd,iret,
     &                 ps=PSREF,pd=pd1,pm=SLREF)
...
! above top
        ...
        SUBROUTINE MOVETX1(IGU,JGU,GLON,GLAT,DATG,DDAT,ZDATG)
       
        PARAMETER (IX=41,JX=41,NSG=8000)

        DIMENSION DATG(IGU,JGU),DDAT(IGU,JGU),ZDATG(IGU,JGU)
...
C234567890123456789012345678901234567890123456789012345678901234567890
        ...
        SUBROUTINE MOVETX(NSEM,ISE,KST,IGU,JGU,GLON,GLAT,DATG,DDAT,
     &                    ZDATG)
...
c        DIMENSION TEST(IGU,JGU)
        ...
        ALLOCATE ( SAVE1(IGU,JGU), SAVE2(IGU,JGU) )
...
!          DO I=IWMIN1,IWMAX1
          ...
          END DO
          END DO
          WRITE(NCHT)((SAVE2(I,J),I=1,IGU),J=1,JGU)   ! ZS
!          DO J=JWMIN1,JWMAX1
!          DO I=IWMIN1,IWMAX1
          DO J=1,JGU
          DO I=1,IGU
          ...
          END DO
          END DO
          WRITE(NCHT)((SAVE2(I,J),I=1,IGU),J=1,JGU)    ! PSL
        END IF
        IF(ISE.EQ.2)THEN
!          DO J=JWMIN1,JWMAX1
!          DO I=IWMIN1,IWMAX1
          ...
          END DO
          END DO
          WRITE(NCHT)((SAVE2(I,J),I=1,IGU),J=1,JGU)    ! T1
        END IF
