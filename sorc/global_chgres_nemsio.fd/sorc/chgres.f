...
!  OPEN  FILES
      ...
      OPEN (NSIGI, FILE='chgres.inp.sig', ACCESS='DIRECT', 
     &      RECL=16, IOSTAT=IRET)
      ...
      IF(IRET == 0 .AND. IRET1 == 0) THEN
        ...
        IF (IRET /= 0) THEN
          INPTYP = 1
          PRINT*,'INPUT ATMOS FILE chgres.inp.sig IS NEMSIO'
        ELSE
          CALL SIGIO_SROPEN(NSIGI,'chgres.inp.sig',IRET)
          CALL SIGIO_SRHEAD(NSIGI,SIGHEADI,IRET1)
          IF (IRET == 0 .AND. IRET1 == 0) THEN
            INPTYP = 2
            PRINT*,'INPUT ATMOS FILE chgres.inp.sig is SIGIO'
          ENDIF
        ENDIF
      ENDIF
...
!  OPEN (READ) NEMSIO GRID FILE HEADERS
     ...
     &,                         IRET=IRET0)
!
!  fv3gfs files do not contain 3-d pressure.  set nopdpvv to true.
...
!       CALL NEMSIO_GETHEADVAR(GFILEI,'LONB', GFSHEADI%LONB,IRET=IRET)
     ...
     &                         IRET=IRET)
        ...
        IF (TRIM(MODELNAME) == 'FV3GFS') GFSHEADI%ICEN2 = 0
     ...
     &                         IRET=IRET)
        IF (TRIM(MODELNAME) == 'FV3GFS') GFSHEADI%PDRYINI = 98.3199
...
!    & 'nrec=',nrec,'gonrec=',GFSHEADO%NREC
        ...
        IF (TRIM(MODELNAME) == "FV3GFS") GFSHEADO%nmetavari = 21
        ...
        IF (TRIM(MODELNAME) == "FV3GFS") GFSHEADO%nmetavarl = 3 
        if(GFSHEADO%nmetavarl > 0) then
          ALLOCATE(GFSHEADVO%varlname(GFSHEADO%nmetavarl))
          ALLOCATE(GFSHEADVO%varlval(GFSHEADO%nmetavarl))
          IF (TRIM(MODELNAME) == "FV3GFS") THEN
            GFSHEADVO%varlname(1) = "HYBRID"
            GFSHEADVO%varlname(2) = "GEN_COORD_HYBRID"
            GFSHEADVO%varlname(3) = "ADIABATIC"
            GFSHEADVO%varlval(1) = .true.
            GFSHEADVO%varlval(2) = .false.
            GFSHEADVO%varlval(3) = .false.
          ELSE
        ...
        IF (TRIM(MODELNAME) == "FV3GFS") GFSHEADO%nmetavarc = 0
        ...
        IF (TRIM(MODELNAME) == "FV3GFS") GFSHEADO%nmetaaryi = 2
        if(GFSHEADO%nmetaaryi > 0) then
          ALLOCATE(GFSHEADVO%aryiname(GFSHEADO%nmetaaryi))
          ALLOCATE(GFSHEADVO%aryilen(GFSHEADO%nmetaaryi))
          IF (TRIM(MODELNAME) == "FV3GFS") THEN
            GFSHEADVO%aryiname(1) = "IDAT"
            GFSHEADVO%aryilen(1)  = 4
            GFSHEADVO%aryiname(2) = "FCSTDATE"
            GFSHEADVO%aryilen(2)  = 7
          ELSE
...
!
     ...
     &                 GFSHEADO%nmetaaryi))
          IF (TRIM(MODELNAME) == "FV3GFS") THEN
            GFSHEADVO%aryival(1,1)  = GFSHEADO%IDATE(4) ! hh
            GFSHEADVO%aryival(2,1)  = GFSHEADO%IDATE(2) ! mm
            GFSHEADVO%aryival(3,1)  = GFSHEADO%IDATE(3) ! dd
            GFSHEADVO%aryival(4,1)  = GFSHEADO%IDATE(1) ! yy
            GFSHEADVO%aryival(:,2)  = GFSHEADO%IDATE(:)
          ELSE
...
!
           ...
           SIGHEADO%fhour = int(GFSHEADO%fhour)
...
!  OPEN SURFACE FILES
      ...
      ELSE
        ...
        CALL NEMSIO_GETFILEHEAD(GFILEISFC,MODELNAME=MODELNAME,
     &       GTYPE=FILETYPE)
...
!
      DO_NSST = .FALSE.
      IF (TRIM(MODELNAME) == "FV3GFS" .AND. NSFCO /= 0) THEN
        DO_NSST = .TRUE.
      ELSE
...
!************************************************************************
      ...
      ELSEIF (INPTYP == 1) THEN ! input data is nemsio format
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!  GET SURFACE HEADER
!--getlpl and zsoil
...
!
        IF(TRIM(MODELNAME) == "FV3GFS") THEN
!  Note: logic hardwired for four soil layers.
     ...
     &           deallocate(GFSHEADVO%RECLEVTYP)
          ...
          ALLOCATE(GFSHEADVO%RECNAME(GFSHEADO%NREC))
          ALLOCATE(GFSHEADVO%RECLEVTYP(GFSHEADO%NREC))
          ALLOCATE(GFSHEADVO%RECLEV(GFSHEADO%NREC))
          GFSHEADVO%RECNAME(1)  = 'alnsf'
          GFSHEADVO%RECNAME(2)  = 'alnwf'
          GFSHEADVO%RECNAME(3)  = 'alvsf'
          GFSHEADVO%RECNAME(4)  = 'alvwf'
          GFSHEADVO%RECNAME(5)  = 'cnwat'
          GFSHEADVO%RECNAME(6)  = 'crain'
          GFSHEADVO%RECNAME(7)  = 'f10m'
          GFSHEADVO%RECNAME(8)  = 'facsf'
          GFSHEADVO%RECNAME(9)  = 'facwf'
          GFSHEADVO%RECNAME(10) = 'ffhh'
          GFSHEADVO%RECNAME(11) = 'ffmm'
          GFSHEADVO%RECNAME(12) = 'fricv'
          GFSHEADVO%RECNAME(13) = 'icec'
          GFSHEADVO%RECNAME(14) = 'icetk'
          GFSHEADVO%RECNAME(15) = 'land'
          GFSHEADVO%RECNAME(16) = 'orog'
          GFSHEADVO%RECNAME(17) = 'salbd'
          GFSHEADVO%RECNAME(18) = 'sfcr'
          GFSHEADVO%RECNAME(19) = 'shdmax'
          GFSHEADVO%RECNAME(20) = 'shdmin'
          GFSHEADVO%RECNAME(21) = 'slc'
          GFSHEADVO%RECNAME(22) = 'slc'
          GFSHEADVO%RECNAME(23) = 'slc'
          GFSHEADVO%RECNAME(24) = 'slc'
          GFSHEADVO%RECNAME(25) = 'sltyp'
          GFSHEADVO%RECNAME(26) = 'smc'
          GFSHEADVO%RECNAME(27) = 'smc'
          GFSHEADVO%RECNAME(28) = 'smc'
          GFSHEADVO%RECNAME(29) = 'smc'
          GFSHEADVO%RECNAME(30) = 'snod'
          GFSHEADVO%RECNAME(31) = 'sotyp'
          GFSHEADVO%RECNAME(32) = 'spfh'
          GFSHEADVO%RECNAME(33) = 'stc'
          GFSHEADVO%RECNAME(34) = 'stc'
          GFSHEADVO%RECNAME(35) = 'stc'
          GFSHEADVO%RECNAME(36) = 'stc'
          GFSHEADVO%RECNAME(37) = 'tg3'
          GFSHEADVO%RECNAME(38) = 'tisfc'
          GFSHEADVO%RECNAME(39) = 'tmp'
          GFSHEADVO%RECNAME(40) = 'tmp'
          GFSHEADVO%RECNAME(41) = 'tprcp'
          GFSHEADVO%RECNAME(42) = 'veg'
          GFSHEADVO%RECNAME(43) = 'vtype'
          GFSHEADVO%RECNAME(44) = 'weasd'
!
          GFSHEADVO%RECLEVTYP       = 'sfc'
          GFSHEADVO%RECLEVTYP(7)    = '10 m above gnd'
          GFSHEADVO%RECLEVTYP(32)   = '2 m above gnd'
          GFSHEADVO%RECLEVTYP(39)   = '2 m above gnd'
          GFSHEADVO%RECLEVTYP(21:24) = 'soil layer'
          GFSHEADVO%RECLEVTYP(26:29) = 'soil layer'
          GFSHEADVO%RECLEVTYP(33:36) = 'soil layer'
!
          GFSHEADVO%RECLEV = 1
          DO K=1,GFSHEADO%NSOIL
            GFSHEADVO%RECLEV(20+K) = K
            GFSHEADVO%RECLEV(25+K) = K
            GFSHEADVO%RECLEV(32+K) = K
          ENDDO
!
        ELSE
     ...
     &           deallocate(GFSHEADVO%RECLEVTYP)
        ...
        IF(TRIM(MODELNAME) == "FV3GFS") GFSHEADO%nmetavari = 4
        GFSHEADO%nmetavarr=GFSHEADI%nmetavarr
        IF(TRIM(MODELNAME) == "FV3GFS") GFSHEADO%nmetavarr = 0
        GFSHEADO%nmetavarr8=GFSHEADI%nmetavarr8
        GFSHEADO%nmetaaryi=GFSHEADI%nmetaaryi
        IF(TRIM(MODELNAME) == "FV3GFS") GFSHEADO%nmetaaryi = 3
        GFSHEADO%nmetaaryr=GFSHEADI%nmetaaryr
        IF(TRIM(MODELNAME) == "FV3GFS") GFSHEADO%nmetaaryr = 1
...
!
     ...
     &                                   deallocate(GFSHEADVO%variname)
          ...
          ALLOCATE(GFSHEADVO%varival(GFSHEADO%nmetavari))
          IF(TRIM(MODELNAME) == "FV3GFS") THEN
            GFSHEADVO%variname(1) = 'latr'
            GFSHEADVO%variname(2) = 'lonr'
            GFSHEADVO%variname(3) = 'lsoil'
            GFSHEADVO%variname(4) = 'ivssfc'
          ELSE
            GFSHEADVO%variname=GFSHEADVI%variname
            GFSHEADVO%varival=GFSHEADVI%varival
          ENDIF
     ...
     &                                  deallocate(GFSHEADVO%aryiname)
          ...
          IF (TRIM(MODELNAME) == 'FV3GFS') then
            GFSHEADVO%aryiname(1) = 'IDAT'
            GFSHEADVO%aryiname(2) = 'LPL'
            GFSHEADVO%aryiname(3) = 'FCSTDATE'
            GFSHEADVO%aryilen(1)  = 4
            GFSHEADVO%aryilen(2)  = (GFSHEADO%LATB+1)/2
            GFSHEADVO%aryilen(3)  = 7
          ELSE
            GFSHEADVO%aryiname=GFSHEADVI%aryiname
            GFSHEADVO%aryilen=GFSHEADVI%aryilen
          ENDIF
     ...
     &      GFSHEADO%nmetaaryi))
          IF (TRIM(MODELNAME) == 'FV3GFS') then
            ...
            GFSHEADVO%aryival(3,1)  = GFSHEADO%IDATE(3) ! dd
            GFSHEADVO%aryival(4,1)  = GFSHEADO%IDATE(1) ! yy
            GFSHEADVO%aryival(:,3)  = GFSHEADO%IDATE(:)
          ELSE
            GFSHEADVO%aryival=GFSHEADVI%aryival
          ENDIF
        endif
     ...
     &                                   deallocate(GFSHEADVO%aryrname)
          ...
          ALLOCATE(GFSHEADVO%aryrlen(GFSHEADO%nmetaaryr))
          IF (TRIM(MODELNAME) == 'FV3GFS') then
            GFSHEADVO%aryrname(1) = 'zsoil'
            GFSHEADVO%aryrlen(1) = GFSHEADO%NSOIL
          ELSE
            GFSHEADVO%aryrname=GFSHEADVI%aryrname
            GFSHEADVO%aryrlen=GFSHEADVI%aryrlen
          ENDIF
...
!read input sfc data
        IF (TRIM(MODELNAME) == "FV3GFS") THEN
          CALL READ_FV3GFS_SFC_DATA_NEMSIO(IMI,JMI,GFSHEADI%NSOIL,
     &                                     GFSDATAI)
        ELSE
...
! output sfcio file
        IF( NSFCO>0 .and. (OUTTYP == 0.or.OUTTYP == 2)) THEN
          ...
          IF(IRET >= 0) THEN
!set sfcio head
            SFCHEADO%fhour=int(GFSHEADO%fhour)
...
!
        ...
        IF (DO_NSST) THEN
          ...
          ALLOCATE(MASK_INPUT(IMI,JMI))
          IF (TRIM(MODELNAME) == "FV3GFS") THEN
            CALL READ_FV3GFS_NSST_DATA_NEMSIO(MASK_INPUT,NSST_INPUT,
     &           IMI, JMI, NUM_NSST_FIELDS, NSST_YEAR,NSST_MON,
     &           NSST_DAY, NSST_HOUR, NSST_FHOUR)
          ELSE
...
!
        ...
        ENDDO
!
      else if(INPTYP ==1) then ! input data is nemsio format
