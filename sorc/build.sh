...
## 03/15/2018 Modified by Xianwu Xue based on Walter Theia Version

set -x -e

mac=$(hostname | cut -c1-1)
mac2=$(hostname | cut -c1-2) # Added by Xianwu Xue based on Walter Theia Version 03/15/2018

#---------------------------------------------------------
if [ $mac2 = tf ]; then # For THEIA # Added by Xianwu Xue based on Walter Theia Version 03/15/2018
	#---------------------------------------------------------
	echo "Building for Theia"
	machine=theia
	ptmp=/scratch4/NCEPDEV/ensemble/ptmp/$LOGNAME

	# Set variables used by makefiles 
  ...
  echo "--Wrong machine! Please use theia, wcoss, cray, zeus to run GEFS!"
