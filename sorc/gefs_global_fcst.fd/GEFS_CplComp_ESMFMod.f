...
!-------------------------------------------
 ...
 DO i = 1, Cpl_Int_State%Total_member
   ...
   CALL ESMF_ConfigGetAttribute(Cf, Cpl_Int_State%pe_member(i), label = PELAB, rc = rc1)

   IF(Cpl_Int_State%pe_member(i) == 0) &
       Cpl_Int_State%pe_member(i) = Cpl_Int_State%nodes / Cpl_Int_State%Total_member
 END DO
