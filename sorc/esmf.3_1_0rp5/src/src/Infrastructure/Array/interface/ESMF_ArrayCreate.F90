! $Id: ESMF_ArrayCreate.cppF90,v 1.22.2.18 2010/02/01 20:49:12 svasquez Exp $
!
...
! totalUWidth, staggerLoc, vectorDim, & 
! name, rc) 
! 
! !ARGUMENTS: 
...
! \item[{[staggerLoc]}] 
! Stagger location is an arbitrary integer index. This information is 
! used to correctly apply connection transformations of the 
! corresponding DistGrid during halo operations. 
! \item[{[vectorDim]}] 
...
! transformations of the corresponding DistGrid during halo operations. 
! \item[{[name]}] 
...
#define ESMF_METHOD "ESMF_ArrayCreateFromPtr" 
 ...
 totalLWidth, totalUWidth, staggerLoc, vectorDim, name=name, rc=localrc) 
 if (ESMF_LogMsgFoundError(localrc, ESMF_ERR_PASSTHRU, & 
 ESMF_CONTEXT, rcToReturn=rc)) return 
...
! !DESCRIPTION: 
! Create an {\tt ESMF\_Array} object from existing local native Fortran 
! arrays according to distgrid. Besides {\tt farray} each PET must issue this 
...
#define ESMF_METHOD "ESMF_ArrayCreateAssmdShape" 
 ...
 allocate(larrayList(1)) 
 fptr => farray 
 larrayList(1) = ESMF_LocalArrayCreate(fptr, copyflag_opt, rc=localrc) 
 if (ESMF_LogMsgFoundError(localrc, ESMF_ERR_PASSTHRU, & 
 ESMF_CONTEXT, rcToReturn=rc)) return 
 ...
 staggerLoc, vectorDim, undistLBound, undistUBound, name, localrc) 
 if (ESMF_LogMsgFoundError(localrc, ESMF_ERR_PASSTHRU, & 
 ESMF_CONTEXT, rcToReturn=rc)) return 
...
! !INTERFACE:
  ...
  function ESMF_ArrayCreateLocalArray(larrayList, distgrid, indexflag, &
    ...
    computationalEdgeUWidth, computationalLWidth, computationalUWidth, &
    totalLWidth, totalUWidth, staggerLoc, vectorDim, undistLBound, &
...
! endif
    ...
    if (present(name)) then
      ...
      call c_ESMC_ArrayCreateLocalArray(array, larrayPointerList, larrayCount, &
        ...
        totalUWidthArg, indexflag, staggerLoc, vectorDim, &
